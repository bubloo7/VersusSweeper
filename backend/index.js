...
app.post("/join", async (req, res) => {
    ...
    // New player, game not full
        ...
        await redis.call("JSON.SET", `game:${id}`, `$.players.${name}`, JSON.stringify(playersElem));
...
app.post("/joinNextGame", async (req, res) => {
    ...
    if (id !== "") {
        ...
        redis.call("JSON.SET", `game:${id}`, ".", JSON.stringify(data)).then(() => {
            ...
            // TODO change expire time later
            redis.call("EXPIRE", `game:${id}`, 60 * 60 * 3);
            redis.call("JSON.SET", `game:${req.body.id}`, `$.nextGameId`, `\"${id}\"`).then(() => {
                res.send({ id });
            });
        });
    }
...
io.on("connection", async (socket) => {
    ...
    socket.on("startGameToServer", () => {
        redis.call("JSON.OBJKEYS", `game:${id}`, "$.players").then((data) => {
            ...
            redis.call("JSON.SET", `game:${id}`, `$.firstMoveName`, `\"${firstMoveName}\"`).then(() => {
                redis.call("JSON.SET", `game:${id}`, `$.gameStarted`, `true`).then(() => {
    ...
    socket.on("firstMoveToServer", async (data) => {
        const checkStartTime = JSON.parse(await redis.call("JSON.GET", `game:${id}`, `$.startTime`));
        if (checkStartTime !== -1) {
            await redis.call("JSON.SET", `game:${id}`, `$.firstColClicked`, `${data.firstColClicked}`);
            await redis.call("JSON.SET", `game:${id}`, `$.firstRowClicked`, `${data.firstRowClicked}`);
            data.startTime = Date.now();
            await redis.call("JSON.SET", `game:${id}`, `$.startTime`, `${data.startTime}`).then();
            io.to(id).emit("firstMoveToClient", data);
        }
    });

    socket.on("revealToServer", async (data) => {
        const hits = await redis.call("JSON.NUMINCRBY", `game:${id}`, `$.players.${data.name}.clears`, data.hits);
        const misses = await redis.call("JSON.NUMINCRBY", `game:${id}`, `$.players.${data.name}.misses`, data.misses);
        ...
        if (data.finishTime !== 1682900908681 * 2) {
            redis.call("JSON.SET", `game:${id}`, `$.players.${data.name}.finishTime`, `${data.finishTime}`);
        }
